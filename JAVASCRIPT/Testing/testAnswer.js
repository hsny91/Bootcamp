function addTwoToEveryNumberInTheList (list) {
  ...
  function getListOfNames(list) {
    ...
    return names;
    /**
     * const names=list.map((person){return person.name})
     * 
  ...
  function getAverageAge(list) {
    ...
    /**
     * const sum=list.reduce((runningSum,cuurentPerson)=>runnindSum+CurrenctPerson.age,0)
     * 
     * 
     */
  }
  ...
  function getAllEngineers(list) {
    ...
    return names;
    /**
     * const enginnering=list.filter(({jobTitle})=>jobTitle.includes("Engineer"))
